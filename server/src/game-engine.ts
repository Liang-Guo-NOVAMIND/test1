...
const SAFE_SQUARES: ReadonlySet<number> = new Set([
...
export class GameEngine {
  ...
  private _consecutiveSixes = 0;
  private _rankings: Colour[] = [];
  ...
  rollDice(): number {
    ...
    if (value === 6) {
      this._consecutiveSixes++;
    } else {
      this._consecutiveSixes = 0;
    }

    if (this._consecutiveSixes >= 3) {
      this._diceValue = null;
      this._consecutiveSixes = 0;
      this._advanceTurn();
      return value;
  ...
  movePiece(pieceIndex: number): MoveResult {
    ...
    if (!this._canMove(piece, this._diceValue))
      throw new Error('Cannot move this piece');
    ...
    if (piece.state === PieceState.YARD) {
      ...
      if (relPos <= homeEntryRel && newRel > homeEntryRel) {
        ...
        piece.trackPosition = this._absoluteFromRelative(
          piece.colour,
          newRel % TRACK_LENGTH
        );
        captured = this._checkCapture(piece, pieceIndex);
    ...
    if (this._checkWin(piece.colour)) {
      ...
      if (!this._rankings.includes(piece.colour)) {
        this._rankings.push(piece.colour);
      }
    }

    const rolledSix = dice === 6;
    const hadCapture = captured !== null;
    this._diceValue = null;

    if (!this._winner) {
      if (hadCapture || rolledSix) {
        // bonus turn — keep same player
      } else {
        this._consecutiveSixes = 0;
        this._advanceTurn();
      }
  ...
  private _checkCapture(
    ...
    for (let i = 0; i < this._pieces.length; i++) {
      ...
      if (
        other.state === PieceState.ACTIVE &&
        other.trackPosition === piece.trackPosition
      ) {
  ...
  private _advanceTurn(): void {
    const count = this._playerCount;
    let next = (this._currentPlayerIndex + 1) % count;
    for (let i = 0; i < count; i++) {
      if (!this._rankings.includes(COLOURS[next]!)) {
